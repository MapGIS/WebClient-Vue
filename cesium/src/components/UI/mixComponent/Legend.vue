...
export default {
  ...
  methods: {
    // 格式化
    getManager() {
      let callback = this.initData.bind(this);
      this.$_init(callback);
    },
    areaString(area) {
    ...
    initData() {
      ...
      let { geoJson, ruleJson } = vm;
      // vm.rules = jsonSource.options.rule;

      vm.rules = ruleJson;

      vm.curcheck = vm.rules.rule.map(sub => sub.title) || [];
      ...
      vm.showtab = vm.echartdata.hasSecond ? true : false;
      vm.allArea = 0;
