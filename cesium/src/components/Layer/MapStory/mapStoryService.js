  ...
  methods: {
            ...
            function (vm) {
              if (index > 0) {
                ...
                vm.$_toggleChapterFeatures(
                  features[index - 1].uuid,
                  features[index - 1].projectUUID,
                  dataSource
                );
              }
              //显示当前章节的所有要素
    ...
    $_play(features, dataSource, callBack) {
      for (let i = 0; i < features.length; i++) {
        this.$_toggleChapterFeatures(
          features[i].uuid,
          features[i].projectUUID,
          dataSource,
          false
        );
      }
    ...
    $_editProject(index) {
      if (this.dataSource instanceof Array) {
        ...
        this.$refs.projectPanel.currentProject =
          this.projectSet[this.dataSource[index].uuid];
      }
    },
    $_closeHoverPanel() {
      let vm = this;
    ...
    $_changeEntity(type, uuid, value) {
      ...
      switch (type) {
        case "fontColor":
          ...
          break;
        case "fontOpacity":
          entity.label.fillColor = Cesium.Color.fromAlpha(
            ...
            entity = vm.viewer.entities.add({
              ...
              label: {
                ...
                fillColor: Cesium.Color.fromCssColorString(
                  feature.feature.layerStyle.color
                ),
              },
            });
            mapStoryManager.options[feature.feature.uuid] = entity;
    ...
    $_setCamera() {
      window.feature.camera.positionCartographic.height =
        this.viewer.camera.positionCartographic.height;
      window.feature.camera.positionCartographic.longitude =
        this.viewer.camera.positionCartographic.longitude;
      window.feature.camera.positionCartographic.latitude =
