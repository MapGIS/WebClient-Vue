    ...
    <div v-show="!editEntity" :style="{height: height + 'px'}" style="position: relative;width: 100%">
      <div v-if="featureCopy" :style="{height: height - 48 + 'px'}" class="mapgis-ui-feature-edit-panel">
        ...
        <mapgis-3d-graphic-single-layer
          ref="graphicLayer"
          :data-source="currentLayer"
          :containerStyle="graphicContainerStyle"
          :iconsPanelStyle="iconsPanelStyle"
          :vueIndex="featureCopy.projectUUID"
          :vueKey="featureCopy.projectUUID"
          @addFeature="$_addFeature"
          @saveCamera="$_getCamera"
          @delete="$_delete"
        />
        <!--富文本-->
        <mapgis-ui-title-row-left
...
export default {
  ...
  watch: {
    feature: {
      handler: function () {
        if(this.projectUUID !== this.feature.projectUUID){
          this.projectUUID = this.feature.projectUUID;
        }
        this.$_init();
  ...
  methods: {
    $_initGraphicLayer(vueIndex, vueKey) {
      this.$refs.graphicLayer.$_init(undefined, vueIndex, vueKey);
    },
    $_changeImage(images) {
      this.featureCopy.images = images;
      this.$emit("change", this.featureCopy);
    },
    $_toggleFillText() {
    ...
    $_addFeature(graphic) {
      this.$emit("addFeature", graphic, this.featureCopy.projectUUID, this.featureCopy.uuid);
    },
    $_showMore() {
    ...
    $_delete(index) {
      this.featureCopy.features.splice(index, 1);
    },
    $_init() {
      ...
      this.currentLayer = JSON.parse(JSON.stringify(this.featureCopy.features));
      if (!layerStyle) {
