    ...
    $_mount() {
      ...
      CesiumZondy.getWebGlobeByInterval(function (webGlobe) {
        vm.$_getM3DByInterval(function (m3d) {
          ...
          CesiumZondy.AnalysisManager.addSource(vueKey,vueIndex instanceof Array ? vueIndex[0] : vueIndex,analysisManager);
          let color = Cesium.Color.fromCssColorString(vm.color);
          color = Cesium.Color.fromAlpha(color,vm.opacity)
          let dynaCuts = [],planes = [];
          for (let i = 0;i < m3d.length; i++){
            //创建剖切对象实例
            let plane = new Cesium.ClippingPlane(new Cesium.Cartesian3(xDirect, yDirect, zDirect), 0.0);
            let dynaCut;
            planes.push(plane);
            if(i === 0){
              dynaCut = analysisManager.createDynamicCutting([m3d[i].source[0]], [planes[i]], {
                color: color
              });
            }else {
            ...
            //设置切面回调函数
            dynaCuts[i].planes[0].plane.plane = new Cesium.CallbackProperty(function (date) {
              if(vm.direction === "left" || vm.direction === "back" || vm.direction === "top"){
                planes[i].distance = vm.endDistanceCopy - vm.currentDistanceCopy;
              }else {
                planes[i].distance = -vm.currentDistanceCopy;
              }
              vm.initSlider = true;
              return Cesium.Plane.transform(plane, m3d[m3dIndex].source[0].modelMatrix, new Cesium.ClippingPlane(Cesium.Cartesian3.UNIT_X, 0.0));
            }, false);
          }
          CesiumZondy.DynamicCuttingManager.addSource(vueKey,vueIndex instanceof Array ? vueIndex[0] : vueIndex,dynaCuts);
        })
      });
    },
    $_unmount(){
