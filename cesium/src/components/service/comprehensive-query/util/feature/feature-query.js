...
export default class FeatureQuery {
    ...
    static isDataStoreQuery(option) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let DNSName = '';
            if (gdbp && gdbp.indexOf('MapGisLocal') === -1 && gdbp.indexOf('@') > -1) {
    ...
    static igsQuery(option, combine, is3D) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!is3D && option.docName && option.layerIdxs !== undefined) {
                ...
                const data = docInfo === null || docInfo === void 0 ? void 0 : docInfo.MapInfos[Number(option.mapIndex) || 0].CatalogLayer;
                const layerName = option.layerName || '';
                const code = option.code || '';
                if (data && option.layerIdxs === '') {
                    option.layerIdxs = DocumentCatalog.getLayerIndexesByNamesOrCodes(data, layerName, code, [], []).join(',');
                }
            }
            if (combine) {
    ...
    static igsQueryFeature(option) {
        ...
        queryParam.rule = new Zondy.MRFS.QueryFeatureRule({
            CompareRectOnly: option.CompareRectOnly !== undefined ? option.CompareRectOnly : false,
            EnableDisplayCondition: option.EnableDisplayCondition !== undefined
        ...
        queryParam.struct = new Zondy.MRFS.QueryFeatureStruct({
            IncludeAttribute: option.IncludeAttribute !== undefined ? option.IncludeAttribute : true,
            IncludeGeometry: option.IncludeGeometry !== undefined ? option.IncludeGeometry : true,
            IncludeWebGraphic: option.IncludeWebGraphic !== undefined
        ...
        if (option.gdbp && !option.docName) {
            ...
            // 图层索引号
            const layerIdxs = option.layerIdxs || option.layerIdxs === '0' ? option.layerIdxs : '*';
            // 时空云文档名
            ...
            queryService = new Zondy.MRFS.QueryDocFeature(queryParam, docName, layerIdxs, {
                domain
            });
        }
    ...
    static igsQuery3DFeature(option) {
        ...
        if (option.gdbp) {
            // 矢量图层
            queryService = new Zondy.G3D.G3DMapDoc(Object.assign({ ip: option.ip, port: option.port, gdbp: option.gdbp }, queryParam));
        }
        else {
            // 矢量文档
            if (!option.docName || !option.layerIdxs) {
            ...
            // 图层索引号
            const layerIdxs = option.layerIdxs || option.layerIdxs === '0' ? option.layerIdxs : '*';
            queryService = new Zondy.G3D.G3DMapDoc(Object.assign({ ip: option.ip, port: option.port, docName, layerindex: layerIdxs }, queryParam));
        }
    ...
    option) {
        ...
        const promise = new Promise((resolve, reject) => {
            new Zondy.PostGIS.PostgisQueryService(queryParam).query((res) => {
                if (!res || !res.features) {
                    reject('undefined');
                }
                else {
