...
export default {
    ...
    "themeOptions.layerStyle": {
      handler: function() {
        ...
        Object.keys(this.themeOptions.layerStyle).forEach(function(key) {
          ...
          if (
            (layerStyleCopy.hasOwnProperty(key) &&
              layerStyle.hasOwnProperty(key) &&
              layerStyleCopy[key] !== layerStyle[key]) ||
            (!layerStyleCopy.hasOwnProperty(key) &&
              layerStyle.hasOwnProperty(key))
          ) {
  ...
  mounted() {
    ...
    if (this.enableLoading) {
      this.$_addSpin(
        this.layerId || "geojson_layer_" + parseInt(Math.random() * 10000)
      );
    }
    this.$_addThemeLayerBySource();
  },
  methods: {
    ...
    $_updateUniqueStyleGroups(groups) {
      ...
      function setPaint(groups, key, vm, keyAlias, field) {
        ...
        let paints = themeManager.getExtraData(
          vm.layerIdCopy,
          vm.themeType,
          vm.dataType + "-" + paintsKey
        );
        if (!paints) {
          let defaultValue;
          switch (vm.dataType) {
          ...
          let pColors = themeManager.getExtraData(
            vm.layerIdCopy,
            vm.themeType,
            vm.dataType + "-color"
          );
      ...
      function setPaint(groups, key, vm, keyAlias) {
        ...
        for (let i = 0; i < paintArr.length; i++) {
          for (let j = 3; j < paints.length; j += 2) {
            if (
              paintArr[i].start <= paints[j] &&
              paints[j] <= paintArr[i].end
            ) {
      ...
      if (vm.themeType !== "heatmap" && vm.themeType !== "symbol") {
        ...
        if (symbolPaintList.hasOwnProperty(key)) {
          if (key === "xOffset" || key === "yOffset") {
            let translate = themeManager.getPanelProps(
              vm.layerIdCopy,
              vm.themeType,
              "icon-translate"
            );
      ...
      if (PaintList.hasOwnProperty(key)) {
        if (key === "color") {
          ...
          if (geojson && geojson instanceof Object) {
            ...
            let colors = this.$_getRangeColors(
              layerStyle[key],
              dataSource,
              this.selectValue,
              geojson.features
    ...
    $_addSpin(layerId) {
      ...
      let spinClassName =
        "mapgis-ui-spin mapgis-ui-spin-spinning mapgis-ui-spin-loading";
      if (spinClass) {
      ...
      this.spinId =
        layerId + "_spin_" + parseInt(String(Math.random() + 10000));
      spin.id = this.spinId;
      ...
      this.maskId =
        layerId + "_mask_" + parseInt(String(Math.random() + 10000));
      mask.id = this.maskId;
    ...
    $_highLightFeature(highlightFeature) {
      ...
      if (this.highlightFeature && this.highlightFeature instanceof Object) {
        if (
          this.highlightFeature.hasOwnProperty("properties") &&
          this.highlightFeature.properties.fid
        ) {
        ...
        if (
          this.highlightFeature.hasOwnProperty("geometry") &&
          this.highlightFeature.geometry.coordinates
        ) {
          this.$_addTips(
            this.$_getCenter(this.highlightFeature),
        ...
        if (this.dataType === "fill") {
          ...
          if (options.highlightStyle) {
            ...
            if (lineStyle) {
              ...
              if (lineStyle.width) {
                heightLightLayer.paint["line-width"] = [
                  "case",
                  ["boolean", ["feature-state", "hover"], false],
                  lineStyle.width,
                  0
            ...
            if (fillStyle) {
              ...
              if (fillStyle.opacity) {
                heightLightLayerFill.paint["fill-opacity"] = [
                  "case",
                  ["boolean", ["feature-state", "hover"], false],
                  fillStyle.opacity,
                  0
          ...
          if (options.highlightStyle) {
            ...
            if (pointStyle) {
              ...
              if (pointStyle.outlineColor) {
                heightLightLayer.paint["circle-stroke-color"] =
                  pointStyle.outlineColor;
              }
              if (pointStyle.outlineWidth) {
                heightLightLayer.paint["circle-stroke-width"] =
                  pointStyle.outlineWidth;
              }
              if (pointStyle.outlineOpacity) {
                heightLightLayer.paint["circle-stroke-opacity"] = [
                  "case",
                  ["boolean", ["feature-state", "hover"], false],
              ...
              if (pointStyle.opacity) {
                heightLightLayer.paint["circle-opacity"] = [
                  "case",
                  ["boolean", ["feature-state", "hover"], false],
                  pointStyle.opacity,
                  0
          ...
          if (options.highlightStyle) {
            ...
            if (lineStyle) {
              ...
              if (lineStyle.opacity) {
                heightLightLayer.paint["line-opacity"] = [
                  "case",
                  ["boolean", ["feature-state", "hover"], false],
                  lineStyle.opacity,
                  0
    ...
    $_addPopup(coordinates, feature) {
      ...
      window.popup.setHTML(
        getPopupHtml(type, feature, this.popupOptions, this.selectValue)
      );
    ...
    $_addTips(coordinates, feature) {
      if (this.enableTips) {
        ...
        window.tips.setHTML(
          getPopupHtml(type, feature, this.tipsOptions, this.selectValue)
        );
    ...
    $_resultToGeojson(result) {
      ...
      for (let i = 0; i < result.SFEleArray.length; i++) {
        let fGeom = result.SFEleArray[i].fGeom,
          feature,
          coordinates;
    ...
    $_addThemeLayerBySource() {
      ...
      if (
        ...
        this.dataSource instanceof Object &&
    ...
    $_addTipsAndPopup(themeType) {
      ...
      if (
        ...
        if (
          ...
          !this.tipsOptions.hasOwnProperty("enableHighlight")
        ) {
          this.tipsSourceId =
            this.layerIdCopy +
            "_tips_" +
      ...
      if (
        ...
        if (
          this.popupOptions.enableHighlight ||
          !this.popupOptions.hasOwnProperty("enableHighlight")
        ) {
          this.popupSourceId =
            this.layerIdCopy +
            "_popup_" +
    ...
    $_addThemeLayer(themeType, layerId, themeField) {
      ...
      let hasManager = themeManager.hasManager(
        this.layerIdCopy + this.$_getThemeName()
      );
      if (hasManager) {
        this.themeType = themeManager.getLayerProps(layerId, "themeType");
        this.selectValue = themeManager.getSelectField(
          this.layerIdCopy,
          this.themeType
        );
        themeManager.field = this.selectValue;
        ...
        this.dataType = themeManager.getLayerProps(
          this.layerIdCopy,
          "dataType"
        );
        this.source_Id = themeManager.getLayerProps(
          this.layerIdCopy,
          this.layerIdCopy
        ).source;
        let props = themeManager.getPanelByField(
          this.layerIdCopy,
          this.themeType,
          this.selectValue
        );
        ...
        this.$refs.themePanel.$_setLabelFields(
          ["未设置"].concat(textFields),
          true
        );
        if (this.themeType === "range") {
          let rangeLevel = themeManager.getExtraData(
            this.layerIdCopy,
            this.themeType,
            "rangeLevel"
          );
        ...
        if (this.themeType === "symbol") {
          let radiusArr = themeManager.getExtraDataByField(
            this.layerIdCopy,
            this.themeType,
            "radiusArr",
            this.selectValue
        ...
        if (this.dataType === "fill" || this.dataType === "line") {
          let patternUrl = themeManager.getPanelProps(
            this.layerIdCopy,
            this.themeType,
        ...
        this.$_getDataByLayer(layerId, function(features) {
          ...
          switch (themeType) {
            case "uniform":
              ...
              themeManager.initExtraData(
                vm.layerIdCopy,
                vm.themeType,
                vm.selectValue
              );
              dataSource = vm.$_setDataSource(
                features,
                vm.selectValue,
                "unique"
              );
            ...
            case "unique":
              ...
              break;
            case "range":
              fields = themeManager.getLayerProps(
                vm.layerIdCopy,
                "numberFields"
              );
              if (vm.initType === "props") {
    ...
    $_setUniqueColors(color, dataSourceCopy, key) {
      if (
        this.themeOptions.layerStyle &&
        this.themeOptions.layerStyle.hasOwnProperty("color")
      ) {
      ...
      if (iSString) {
        ...
        if (this.themeOptions.styleGroups) {
          ...
          for (let i = 0; i < this.themeOptions.styleGroups.length; i++) {
            if (
              newColors.indexOf(this.themeOptions.styleGroups[i].value) >= 0
            ) {
              newColors[
                newColors.indexOf(this.themeOptions.styleGroups[i].value) + 1
            ...
            if (this.themeOptions.styleGroups[i].style.radius) {
              ...
              if (radius.indexOf(this.themeOptions.styleGroups[i].value) >= 0) {
                radius[
                  radius.indexOf(this.themeOptions.styleGroups[i].value) + 1
            ...
            if (this.themeOptions.styleGroups[i].style.outlineWidth) {
              if (!outlineWidth) {
                ...
                outlineWidth = formatInterpolate(
                  iSString,
                  dataSourceCopy,
                  key,
                  w
              ...
              if (
                outlineWidth.indexOf(this.themeOptions.styleGroups[i].value) >=
                0
              ) {
                outlineWidth[
                  outlineWidth.indexOf(this.themeOptions.styleGroups[i].value) +
            ...
            if (this.themeOptions.styleGroups[i].style.outlineColor) {
              if (!outlineColor) {
                ...
                outlineColor = formatInterpolate(
                  iSString,
                  dataSourceCopy,
                  key,
                  c
              ...
              if (
                outlineColor.indexOf(this.themeOptions.styleGroups[i].value) >=
                0
              ) {
                outlineColor[
                  outlineColor.indexOf(this.themeOptions.styleGroups[i].value) +
            ...
            if (this.themeOptions.styleGroups[i].style.outlineOpacity) {
              if (!outlineOpacity) {
                ...
                outlineOpacity = formatInterpolate(
                  iSString,
                  dataSourceCopy,
                  key,
                  o
              ...
              if (
                outlineOpacity.indexOf(
                  this.themeOptions.styleGroups[i].value
                ) >= 0
              ) {
                outlineOpacity[
                  outlineOpacity.indexOf(
            ...
            if (
              this.themeOptions.styleGroups[i].style.hasOwnProperty("opacity")
            ) {
              if (!opacity) {
              ...
              if (
                opacity.indexOf(this.themeOptions.styleGroups[i].value) >= 0
              ) {
                opacity[
                  opacity.indexOf(this.themeOptions.styleGroups[i].value) + 1
        ...
        if (this.themeOptions.styleGroups) {
          for (let i = 0; i < this.themeOptions.styleGroups.length; i++) {
            ...
            if (
              this.themeOptions.styleGroups[i].style.hasOwnProperty("radius")
            ) {
              if (!radius) {
                let r = this.themeOptions.layerStyle.radius || 6;
                radius = formatInterpolate(iSString, dataSourceCopy, key, r);
            ...
            if (
              this.themeOptions.styleGroups[i].style.hasOwnProperty(
                "outlineWidth"
              )
            ) {
              if (!outlineWidth) {
    ...
    $_getRangeColors(color, dataSourceCopy, key, features) {
      ...
      let colors = ["step", ["to-number", ["get", key]]],
        checkBoxArr = [],
    ...
    $_setRangeColors(color, dataSourceCopy, key, features) {
      let colorAndCheckBox = this.$_getRangeColors(
        color,
        dataSourceCopy,
        key,
        features
      ...
      //如果有styleGroups，则应用
      if (
        this.themeOptions.styleGroups &&
        this.themeOptions.styleGroups.length > 0
      ) {
        //处理透明度
        let opacity = 1;
        const { layerStyle } = this.themeOptions;
        ...
        for (let i = 0; i < this.themeOptions.styleGroups.length; i++) {
          if (
            this.themeOptions.styleGroups[i].hasOwnProperty("style") &&
            this.themeOptions.styleGroups[i].hasOwnProperty("start") &&
            this.themeOptions.styleGroups[i].hasOwnProperty("end")
          ) {
            if (
              this.themeOptions.styleGroups[i].style.hasOwnProperty("color")
            ) {
              for (let j = 3; j < colors.length; j += 2) {
                if (
                  this.themeOptions.styleGroups[i].start <= colors[j] &&
                  colors[j] <= this.themeOptions.styleGroups[i].end
                ) {
            ...
            if (
              ...
              for (let j = 3; j < outlineOpacities.length; j += 2) {
                if (
                  this.themeOptions.styleGroups[i].start <=
                    outlineOpacities[j] &&
          ...
          if (range === 0) {
            ...
            for (let j = 0; j < this.rangeLevel; j++) {
              newDataSourceCopy.push(
                Number(dataSourceCopy[0]) + (j + 1) * rangeSect
              );
            }
    ...
    $_addSymbolLayer(dataSource) {
      ...
      img.addEventListener("load", function() {
        ...
        if (vm.map.hasImage(iconName)) {
          ...
          let newIconSize;
          if (
            vm.themeOptions.styleGroups &&
            vm.themeOptions.styleGroups.length > 0
          ) {
    ...
    $_addTextLayer() {
      ...
      if (enableIgs && baseUrl && this.dataType === "fill") {
        ...
        for (let i = 0; i < features.length; i++) {
          ...
          let url =
            baseUrl +
            "/igs/rest/mrfs/getLabel?geometryType=polygon&f=json&geometry=" +
            geometryStr;
          FeatureService.get(
            url,
        ...
        let textInterval = setInterval(function() {
          if (featureLength === textGeojson.features.length) {
            ...
            if (
              !vm.map.getLayer(vm.layerIdCopy + vm.$_getThemeName() + "_注记")
            ) {
        ...
        if (
          !this.map.getLayer(this.layerIdCopy + this.$_getThemeName() + "_注记")
        ) {
    ...
    $_addLineLayer() {
      ...
      if (
        !this.map.getLayer(this.layerIdCopy + this.$_getThemeName() + "_线")
      ) {
    ...
    $_setLayerOrder(order) {
      ...
      //单个图层的专题图顺序数组
      let themeOrder = themeManager.getLayerProps(
        this.layerIdCopy,
        "layerOrder"
      );
      if (!themeOrder) {
      ...
      Object.keys(originLayer).forEach(function(key) {
        switch (key) {
          ...
          default:
            if (
              originLayer[key] &&
              !(originLayer[key] instanceof Function) &&
              key.indexOf("_") < 0
            ) {
    ...
    $_themeTypeChanged(themeType) {
      ...
      this.themeType = themeType;
      //取得选中字段
      let selectValue = themeManager.getSelectField(
        this.layerIdCopy,
        this.themeType
      );
      //面板参数
      ...
      if (!props) {
        ...
        //设置管理器字段
        themeManager.field = vm.selectValue;
        let pattern = themeManager.getPanelProps(
          this.layerIdCopy,
          this.themeType,
        ...
        if (this.themeType !== "heatmap") {
          dataSource = themeManager.getExtraData(
            this.layerIdCopy,
            this.themeType,
            "dataSource"
          );
    ...
    $_fieldChanged(field) {
      ...
      if (!props) {
        ...
        this.$_getDataByLayer(this.layerIdCopy, function(features) {
          ...
          switch (vm.themeType) {
            case "uniform":
              ...
              //设置专题图颜色
              vm.currentGradient = "#EE4C5A";
              paintColor = vm.$_setUniformColors(
                vm.currentGradient,
                dataSource,
                field
              );
              vm.$_setPaintPropertyToExtra(
                vm.layerIdCopy,
                vm.layerIdCopy + vm.$_getThemeName(),
                vm.dataType + "-color",
                paintColor
            ...
            case "unique":
              ...
              //设置专题图颜色
              vm.currentGradient = gradients[0].key;
              paintColor = vm.$_setUniqueColors(
                vm.currentGradient,
                dataSource,
                field
              );
          ...
          //取得初始化的面板参数
          props = themeManager.getPanelByField(
            vm.layerIdCopy,
            vm.themeType,
            field
          );
        ...
        if (this.themeType === "uniform") {
          if (!hasPatternUrl) {
            //设置专题图颜色
            paintColor = themeManager.getExtraData(
              this.layerIdCopy,
              this.themeType,
              this.dataType + "-color"
            );
            this.$_setPaintPropertyToExtra(
              this.layerIdCopy,
              this.layerIdCopy + this.$_getThemeName(),
              this.dataType + "-color",
              paintColor
          ...
          this.$_getDataByLayer(this.layerIdCopy, function(features) {
            ...
            vm.$refs.themePanel.$_setDataSoure(dataSource);
            //设置radiusIndex值
            let radiusIndex = themeManager.getPanelProps(
              vm.layerIdCopy,
              vm.themeType,
              "radiusIndex"
            );
          ...
          themeManager.setLayerPaintProperty(
            vm.layerIdCopy,
            vm.layerIdCopy + vm.$_getThemeName(),
            "heatmap-opacity",
            opacity
          );
          let heatmapWeight = themeManager.getExtraData(
            this.layerIdCopy,
            this.themeType,
            "heatmap-weight"
          );
          ...
          let heatmapColor = themeManager.getExtraData(
            this.layerIdCopy,
            this.themeType,
            "heatmap-color"
          );
          ...
          let heatmapRadius = themeManager.getExtraData(
            this.layerIdCopy,
            this.themeType,
            "heatmap-radius"
          );
        ...
        case "fill":
          Object.keys(props).forEach(function(key) {
            ...
            if (
              ...
              key.indexOf("fill-") > -1 &&
              key.indexOf("fill-outline-") < 0 &&
              key !== vm.dataType + "-color" &&
              key !== "fill-pattern-url"
            ) {
              //处理线偏移
              if (key === "fill-translate") {
                vm.$_setPaintProperty(
                  ...
                  vm.layerIdCopy + vm.$_getThemeName() + "_线",
                  "line-translate",
                  [Number(value[0]), Number(value[1]) * -1]
                );
                vm.$_setPaintProperty(
                  vm.layerIdCopy,
                  vm.layerIdCopy + vm.$_getThemeName(),
                ...
                if (vm.map.hasImage(value)) {
                  ...
                  let img = new Image(16, 16);
                  let fillPatternUrl = themeManager.getPanelProps(
                    vm.layerIdCopy,
                    vm.themeType,
                    "fill-pattern-url"
                  );
          ...
          if (vm.themeType === "symbol") {
            Object.keys(props).forEach(function(key) {
              if (
                props.hasOwnProperty(key) &&
                key !== "icon-url" &&
                (key.indexOf("text-") > -1 || key.indexOf("icon-") > -1)
              ) {
                let value = props[key];
                if (key === "icon-image") {
                  let iconName = themeManager.getPanelProps(
                    vm.layerIdCopy,
                    vm.themeType,
                    "icon-image"
                  );
                  if (vm.map.hasImage(iconName)) {
                    ...
                    img.src = themeManager.getPanelProps(
                      vm.layerIdCopy,
                      vm.themeType,
                      "icon-url"
                    );
    ...
    $_rangeInputChanged(index, data) {
      if (this.themeType === "symbol") {
        let radiusArr = themeManager.getExtraData(
          this.layerIdCopy,
          this.themeType,
          "radiusArr"
        );
        ...
        let colors = themeManager.getPanelProps(
          this.layerIdCopy,
          this.themeType,
          "colors"
        );
        ...
        if (index === 0) {
          let startData = themeManager.getPanelProps(
            this.layerIdCopy,
            this.themeType,
            "startData"
          );
    ...
    $_setRangeColor(color, startData, endData) {
      let paintColor = themeManager.getExtraData(
        this.layerIdCopy,
        this.themeType,
        this.dataType + "-color"
      );
      ...
      for (let i = 0; i < length; i++) {
        if (
          paintColor[2 + i * 2 + 1] >= startData &&
          paintColor[2 + i * 2 + 1] <= endData
        ) {
          if (i === 0) {
    ...
    $_setIconSize(dataSourceCopy, radiusArr) {
      ...
      iconSize.push(0.1);
      this.$_setLayOutPropertyToExtra(
        this.layerIdCopy,
        this.layerIdCopy + this.$_getThemeName(),
        "icon-size",
        iconSize
    ...
    $_editTranslate(translate, index) {
      let translateArr;
      if (this.themeType === "symbol") {
        translateArr = themeManager.getPanelProps(
          this.layerIdCopy,
          this.themeType,
          "icon-translate"
        );
    ...
    $_yCircleTranslateChanged(translate) {
      ...
      let arr = themeManager.getPanelProps(
        this.layerIdCopy,
        this.themeType,
        "circle-translate"
      );
    ...
    $_textOffsetXChanged(offset) {
      let textOffset = themeManager.getPanelProps(
        this.layerIdCopy,
        this.themeType,
        "text-offset"
      );
      ...
      if (paintColors instanceof Array) {
        switch (this.themeType) {
          ...
          case "range":
            this.$_setRangeColor(
              color,
              this.$_getStartEndData(index).startData,
              this.$_getStartEndData(index).endData
            );
            break;
        }
      } else {
    ...
    $_checked(checkBoxArr, index, color) {
      ...
      //存储checkBoxArr
      let newCheckBoxArr = themeManager.getPanelProps(
        this.layerIdCopy,
        this.themeType,
        "checkBoxArr"
      );
    ...
    $_gradientChanged(color) {
      ...
      switch (this.themeType) {
        case "unique":
          //切换渐变颜色
          paintColor = this.$_setUniqueColors(
            color,
            this.dataSourceCopy,
            this.selectValue
          );
    ...
    $_getStartEndData(index) {
      ...
      if (index === 0) {
        startData = themeManager.getPanelProps(
          this.layerIdCopy,
          this.themeType,
          "startData"
        );
    ...
    $_getHeatmapRadius(radius) {
      return ["interpolate", ["linear"], ["zoom"], 0, radius, 9, radius];
    },
    ...
    $_getAllLayer() {
      let allLayer = themeManager.getAllLayer(),
        layerNameArr = [];
      ...
      let interval = setInterval(function() {
        ...
        if (initAllSource) {
          ...
          for (let j = 0; j < originLayerIds.length; j++) {
            ...
            let dataType = themeManager.getLayerProps(
              originLayerIds[j],
              "dataType"
            );
            vm.map.setPaintProperty(
          ...
          for (let i = 0; i < layerOrder.length; i++) {
            if (layerOrder[i].indexOf("专题图") < 0) {
              ...
              let beforeLayer =
                layerNameArr[layerNameArr.indexOf(originLayerId) + 1];
              if (!vm.map.getLayer(layerOrder[i])) {
                if (layerOrder[i].indexOf("_符号专题图") > -1) {
                  let img = new Image(128, 128);
                  let field = themeManager.getSelectField(
                    originLayerId,
                    "symbol"
                  );
                  let iconUrl = themeManager.getPanelPropsByField(
                    originLayerId,
                    "symbol",
                    "icon-url",
                    field
                  );
                  let iconName = themeManager.getPanelPropsByField(
                    originLayerId,
                    "symbol",
                    "icon-image",
                    field
                  ...
                  img.addEventListener("load", function() {
                    ...
                    if (hasIcon) {
                      vm.map.addLayer(
                        themeManager.getLayer(symbolOriginId, symbolId),
                        beforeLayer
                      );
                    }
                  });
                  ...
                  if (
                    (dataType === "fill" || dataType === "line") &&
                    layerOrder[i].indexOf("_注记") < 0 &&
                    layerOrder[i].indexOf("_线") < 0
                  ) {
                    let themeType = themeManager.getLayerProps(
                      originLayerId,
                      "themeType"
                    );
                    let field = themeManager.getSelectField(
                    ...
                    let pattern = themeManager.getPanelPropsByField(
                      originLayerId,
                      themeType,
                      dataType + "-pattern",
                      field
                    );
                    let patternUrl = themeManager.getPanelPropsByField(
                      originLayerId,
                      themeType,
                      dataType + "-pattern-url",
                      field
                    );
                    vm.$refs.themePanel.$_setIcons(vm.icons);
                    if (pattern && !vm.map.hasImage(pattern)) {
                      ...
                      vm.map.addLayer(
                        themeManager.getLayer(originLayerId, layerOrder[i]),
                        beforeLayer
                      );
                    }
                  } else {
    ...
    $_resetAllLayer(layerId) {
      let layerOrder = themeManager.getLayerProps(layerId, "layerOrder"),
        vm = this;
      ...
      allLayerOrder.splice(
        allLayerOrder.indexOf(layerOrder[0]),
        layerOrder.length
      );
      ...
      if (
        originLayer.hasOwnProperty("paint") &&
        originLayer.paint.hasOwnProperty(dataType + "-opacity")
      ) {
    ...
    $_removeHighlightLayer(name) {
      if (
        this.map.getLayer(
          this.layerIdCopy + this.$_getThemeName() + name + "_高亮_点"
        )
      ) {
      ...
      if (
        this.map.getLayer(
          this.layerIdCopy + this.$_getThemeName() + name + "_高亮_线"
        )
      ) {
        this.map.removeLayer(
          this.layerIdCopy + this.$_getThemeName() + name + "_高亮_线"
      ...
      if (
        this.map.getLayer(
          this.layerIdCopy + this.$_getThemeName() + name + "_高亮_多边形"
        )
      ) {
        this.map.removeLayer(
          this.layerIdCopy + this.$_getThemeName() + name + "_高亮_多边形"
    ...
    $_hideCurrentLayer(layerId) {
      ...
      for (let i = 0; i < layerOrder.length; i++) {
        if (
          layerOrder[i].indexOf(layerId + this.$_getThemeName(themeType)) > -1
        ) {
          this.$_setLayOutProperty(
            layerId,
            layerOrder[i],
    ...
    $_getLegend(layerId) {
      ...
      switch (this.themeType) {
        case "uniform":
          colors = themeManager.getExtraData(
            layerId,
            this.themeType,
            this.dataType + "-color"
          );
        ...
        case "heatmap":
          ...
          break;
        case "symbol":
          colors = themeManager.getPanelProps(
            layerId,
            this.themeType,
            "icon-url"
          );
