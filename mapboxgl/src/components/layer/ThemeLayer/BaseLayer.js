    ...
    themeOptions: {
      handler: function() {
        if (this.themeOptions instanceof Array) {
          ...
          if (
            this.themeOptions[0] &&
            this.themeOptions[0] instanceof Object &&
            this.themeOptions[0].hasOwnProperty("max")
          ) {
            ...
            vm.$_getDataByLayer(vm.layerIdCopy, function(features) {
              //切换渐变颜色
              let paintColor = vm.$_setRangeColors(
                options.colors.join(","),
                options.dataSource,
                vm.selectValue,
                features
      ...
      handler: function() {
        ...
        for (let i = 0; i < rects.length; i++) {
          ...
          for (let j = 0; j < rows.length; j++) {
            let oldData = vm.$_getValueById(
              rows[j].id,
              "value",
              vm.themePropsCopy.options
            );
            let newData = vm.$_getValueById(
              rows[j].id,
              "value",
              vm.themeProps.options
            );
            if (newData !== oldData) {
  ...
  methods: {
    ...
    $_addThemeLayerBySource() {
      if (this.dataSource) {
        ...
        this.initType = "props";
        let layerId =
          this.themeProps.layerId ||
          "geojson_layer_" + parseInt(Math.random() * 10000);
        if (
          this.themeProps.panelClass &&
          this.themeProps.panelClass instanceof Array
        ) {
          this.panelClass = {};
        ...
        this.$_addThemeLayer(
          this.themeProps.themeType,
          layerId,
          this.themeProps.themeField
        );
