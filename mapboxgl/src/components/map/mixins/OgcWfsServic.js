import {IgsServiceBase} from "@mapgis/webclient-es6-service/baseserver";
...
export default {
  ...
  mounted() {
    //取得地图文档id
    this.mapId = this._url.substr(this._url.indexOf("doc") + 4,this._url.indexOf("/WFSServer") - this._url.indexOf("doc") - 4);
  },
  methods: {
    get(url, fn,error){
    ...
    _xmlToResult(xmlObj,mapId){
      ...
      Object.keys(xmlObj).forEach(function (key) {
        //取得attributeName
        let attributeName = key.substr(mapId.length + 1),posList;
        result[attributeName] = [];
        //如果返回的为Reg即多边形坐标，进行处理
        if(key === mapId + ":Reg"){
          //取得数据
          let surfaceMember = xmlObj[key]["gml:MultiSurface"]["gml:surfaceMember"];
          if(surfaceMember instanceof Array){
            let polygons = [];
            for (let i = 0;i < surfaceMember.length;i++){
              let polygon = {};
              if(surfaceMember[i]["gml:Polygon"].hasOwnProperty("gml:exterior")){
                let exteriorPos = surfaceMember[i]["gml:Polygon"]["gml:exterior"]["gml:LinearRing"]["gml:posList"].split(" ");
                polygon.exterior = [];
                for (let i = 0;i <= exteriorPos.length;i += 2){
                  polygon.exterior.push([Number(exteriorPos[i]),Number(exteriorPos[i + 1])]);
                }
              }
              if(surfaceMember[i]["gml:Polygon"].hasOwnProperty("gml:interior")){
                let interiorPos = surfaceMember[i]["gml:Polygon"]["gml:interior"]["gml:LinearRing"]["gml:posList"].split(" ");
                polygon.interior = [];
                for (let i = 0;i <= interiorPos.length;i += 2){
                  polygon.interior.push([Number(interiorPos[i]),Number(interiorPos[i + 1])]);
                }
              }
            ...
            posList = surfaceMember["gml:Polygon"]["gml:exterior"]["gml:LinearRing"]["gml:posList"].split(" ");
            //重新拼装
            for (let i = 0;i < posList.length;i += 2){
              result[attributeName].push([Number(posList[i + 1]),Number(posList[i + 1])]);
            }
          }
          ...
          //如果返回的为Lin即线坐标，进行处理
          //取得坐标数据
          posList = xmlObj[key]["gml:MultiCurve"]["gml:curveMember"]["gml:LineString"]["gml:posList"].split(" ");
          result[attributeName] = [];
          for (let i = 0;i < posList.length;i += 2){
            result[attributeName].push([Number(posList[i]),Number(posList[i + 1])]);
          }
        }else {
    ...
    _query(param,onSuccess,onError){
      //拼接GetFeature的url
      let wfsUrl = this._url + "&REQUEST=GetFeature",vm = this;
      //转换参数为WFS需要的的参数
        ...
        Object.keys(featureObjXML).forEach(function (key) {
          ...
          if(featureObjXML[key] instanceof Array){
            ...
            for (let i = 0;i < featureObjXML[key].length;i++){
              //将结果转换为Object形式
              results[layerId].push(vm._xmlToResult(featureObjXML[key][i],vm.mapId));
            }
          }else {
            //当查询到的结果为一个时，返回对象
            results[layerId].push(vm._xmlToResult(featureObjXML[key],vm.mapId));
          }
        });
        if(onSuccess){
          onSuccess(results);
    ...
    $_addWFS(features,layer,onSuccess,onError){
      ...
      let xml = "<wfs:Transaction xmlns='http://www.someserver.com/myns' xmlns:gml='http://www.opengis.net/gml'   xmlns:" + this.mapId + "='http://www.mapgis.com.cn'  xmlns:wfs='http://www.opengis.net/wfs' xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance' xsi:schemaLocation='http://www.someserver.com/myns http://www.someserver.com/wfs/cwwfs.cgirequest=describefeaturetype http://www.opengis.net/wfs ../wfs/1.0.0/WFS-transaction.xsd'>",vm = this;
      for(let i = 0;i < features.length;i++){
        ...
        if(features[i].geometry.type === "Point"){
          xml += "<gml:coordinates>" + pos[0] + " " + pos[1] + "</gml:coordinates>";
        ...
        Object.keys(features[i].attributes).forEach(function (key) {
          xml += "<" + vm.mapId + ":" + key + ">" + features[i].attributes[key] + "</" + vm.mapId + ":" + key + ">";
        });
    ...
    $_updateWFS(features,layer,onSuccess,onError){
      let xml = "<wfs:Transaction xmlns='http://www.someserver.com/myns' xmlns:gml='http://www.opengis.net/gml' xmlns:ogc='http://www.opengis.net/ogc'   xmlns:" + this.mapId + "='http://www.mapgis.com.cn'  xmlns:wfs='http://www.opengis.net/wfs' xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance' xsi:schemaLocation='http://www.someserver.com/myns http://www.someserver.com/wfs/cwwfs.cgirequest=describefeaturetype http://www.opengis.net/wfs ../wfs/1.0.0/WFS-transaction.xsd'>";
      let vm = this;
      for (let i = 0;i < features.length;i++){

      }
      xml += "<wfs:Update typeName='" + this.mapId + ":" + layer + "'><wfs:Property>";
      console.log(features[0])
      Object.keys(features[0].attributes).forEach(function (key) {
        xml += "<wfs:" + key + ">" + vm.mapId + ":" +features[0].attributes[key] + "</wfs:" + key + ">";
      });
      xml += "</wfs:Property>";
      xml += "<ogc:Filter><ogc:GmlObjectId gml:id='" + layer + ".163'/></ogc:Filter>";
