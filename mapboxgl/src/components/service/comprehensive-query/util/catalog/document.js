...
export default class DocumentCatalog {
    ...
    static getDocInfo(option) {
        ...
        const url = `${domain}/igs/rest/mrcs/docs/${option.serverName}?dataService=${option.serverName}&f=json&tree=true&guid=${option.guid ||
            '__readonly_user__'}`;
        const self = this;
        const promise = new Promise((resolve, reject) => {
            axios.get(url).then(res => {
                ...
                if (!data) {
                    ...
                    for (let i = 0; i < data.MapInfos.length; i += 1) {
                        ...
                        if (CatalogLayer) {
                            data.MapInfos[i].CatalogLayer = self.disposeDocInfo(CatalogLayer);
                        }
                    }
    ...
    static disposeDocInfo(vectorInfo, layerIndex) {
        ...
        for (let i = 0; i < vectorInfo.length; i += 1) {
            const tip = layerIndex && layerIndex !== '' ? `${layerIndex}-${i}` : String(i);
            if (vectorInfo[i].Type === 'Layer') {
                ...
                if (groupMapLayerInfo) {
                    vectorInfo[i].GroupMapLayerInfo = this.disposeDocInfo(groupMapLayerInfo, tip);
                }
            }
        }
        return vectorInfo;
     ...
     * @returns                   图层索引列表
     */
    static getLayerIndexesByNamesOrCodes(data, layerName = '', code = '', array = [], urlArray = []) {
        array = array || [];
        urlArray = urlArray || [];
        if (layerName !== '' || code !== '') {
            for (let i = 0; i < data.length; i += 1) {
                ...
                if (data[i].Type === 'Layer') {
                    ...
                    array = this.getLayerIndexesByNamesOrCodes(GroupMapLayerInfo, layerName, code, array, urlArray);
                }
            }
        }
        else {
        ...
        if (layerName !== '' || code !== '') {
            for (let i = 0; i < data.length; i += 1) {
                ...
                if (data[i].Type === 'Layer') {
                    ...
                    urlArray = this.getLayerUrlsByNamesOrCodes(GroupMapLayerInfo, layerName, code, urlArray);
                }
            }
        }
        else {
     ...
     */
    static getLayersByIndexes(layerIdxs, sourceLayerInfoArray, destLayerInfoArray = []) {
        // 图层索引号数组
        ...
        for (let i = 0; i < sourceLayerInfoArray.length; i += 1) {
            ...
            if (sourceLayerInfoArray[i].Type === 'Group') {
                ...
                if (GroupMapLayerInfo) {
                    destLayerInfoArray = this.getLayersByIndexes(layerIdxs, GroupMapLayerInfo, destLayerInfoArray);
                }
            }
        }
    ...
    static getLayerIndexesByNames(data, names, urlArray) {
        ...
        for (let i = 0; i < data.length; i += 1) {
            if (data[i].Type === 'Layer') {
                ...
                if (GroupMapLayerInfo) {
                    urlArray = this.getLayerIndexesByNames(GroupMapLayerInfo, names, urlArray);
                }
            }
        }
        return urlArray;
            ...
            if (data[i].Type === 'Layer') {
                ...
                if (layerNames) {
                    for (let j = 0; j < layerNames.length; j++) {
                        if (data[j].URL.indexOf(layerNames[j]) ===
                ...
                array = this.getLayersByExtentOrName(data[i].GroupMapLayerInfo, extent, layerNames, array, gdbps);
            }
        }
        return array;
    }
                ...
                tileLayer.getTileInfo(res => {
                    if (!res || !res.TileInfo2) {
                        ...
                        if (fullExtent) {
                            ...
                            const proName = res.TileInfo2.tileInfo.spatialReference.tileSRefInfo.Name;
                            const tileSize = res.TileInfo2.tileInfo.cols || res.TileInfo2.tileInfo.rows;
