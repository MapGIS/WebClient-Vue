...
function getPopupContainer(popupStyle) {
    return "<div class='mapgis-popup-container " + popupStyle.containerClass + "' style='" + popupStyle.containerStyle + "'>";
}

function getPopupTitle(popupStyle, title, rowContainerClass) {
    rowContainerClass = "mapgis-popup-row-container " + rowContainerClass;
    return "<div class='mapgis-popup-title " + popupStyle.titleClass + "' style='" + popupStyle.titleStyle + "'>" + title + "</div><div class='" + rowContainerClass + "'>";
}

function getPopupRows(fields, alias, popupStyle, feature, defaultField, rowClass, fieldClass, valueClass, before) {
    let field, element = "";
    before = before || "";
    function getRow(popupStyle, field, value){
        return "<div class='" + rowClass + " " + popupStyle.rowClass + "' style='" + popupStyle.rowStyle + "'>" + before + "<span class='" + fieldClass + " " + popupStyle.fieldClass + "' style='" + popupStyle.fieldStyle + "' title='" + field + "'>" + field + "</span><span class='" + valueClass + " " + popupStyle.valueClass + "' style='" + popupStyle.valueStyle + "' title='" + value + "'>" + value + "</span></div>";
    }
...
function getDefaultPopup(fields, alias, style, pClass, title, feature, defaultField, popupStyle, rowClass, fieldClass, valueClass, before) {
...
function getPopupLeftHtml(fields, alias, style, pClass, title, feature, defaultField, popupStyle) {
    let element = getPopupContainer(popupStyle), field;
    if (title) {
        element += getPopupTitle(popupStyle, title, "mapgis-popup-row-left-container");
    }
    function getRow(popupStyle, field, value) {
        return "<div class='mapgis-popup-row mapgis-popup-left-row " + popupStyle.rowClass + "' style='" + popupStyle.rowStyle + "'><div class='mapgis-popup-left-item mapgis-popup-left-field " + popupStyle.fieldClass + "' style='" + popupStyle.fieldStyle + "' title='" + field + "'>" + field + "</div><div class='mapgis-popup-left-item mapgis-popup-left-value mapgis-popup-ellipsis " + popupStyle.valueClass + "' style='" + popupStyle.valueStyle + "' title='" + value + "'>" + value + "</div></div>";
    }
    //特例不提取
    if (fields && fields instanceof Array && fields.length > 0) {
        for (let i = 0; i < fields.length; i++) {
...
export function getPopupHtml(type, feature, options, currentField){
    ...
    switch (type) {
        case "left":
            element = getPopupLeftHtml(options.fields, options.alias, options.style, options.class, options.title, feature, currentField, popupStyle);
            break;
        case "underline":
            element = getDefaultPopup(options.fields, options.alias, options.style, options.class, options.title, feature, currentField, popupStyle, "mapgis-popup-underline-row", "mapgis-popup-item mapgis-popup-underline-item mapgis-popup-field mapgis-popup-underline-field", "mapgis-popup-item mapgis-popup-underline-item mapgis-popup-value");
            break;
        case "point":
            element = getDefaultPopup(options.fields, options.alias, options.style, options.class, options.title, feature, currentField, popupStyle, "mapgis-popup-point-row", "mapgis-popup-item mapgis-popup-point-item mapgis-popup-field mapgis-popup-point-field", "mapgis-popup-item mapgis-popup-point-item mapgis-popup-value mapgis-popup-point-value", "<span class='mapgis-popup-point'></span>");
            break;
        case "table":
            element = getDefaultPopup(options.fields, options.alias, options.style, options.class, options.title, feature, currentField, popupStyle, "mapgis-popup-table-row", "mapgis-popup-item mapgis-popup-table-item mapgis-popup-field mapgis-popup-table-field", "mapgis-popup-item mapgis-popup-table-item mapgis-popup-value mapgis-popup-table-value");
            break;
        case "default":
