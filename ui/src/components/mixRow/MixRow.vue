  ...
  <div>
    ...
    <mapgis-ui-row class="mix-row" v-if="type === 'MapgisUiGrediantSelect'">
      ...
      <mapgis-ui-col :span="grediantPickerProps.selectCol">
        ...
        >
          <mapgis-ui-select-option
            v-for="(gradient, index) in dataSource"
            :key="index"
            :value="gradient.key"
          >
            <div
              class="mix-row-gradient"
              :style="{
                background: gradient.value,
                ...grediantPickerProps.optionStyle
              }"
            />
          </mapgis-ui-select-option>
        </mapgis-ui-select>
    ...
    <mapgis-ui-row
      ...
      v-if="
        type === 'MapgisUiThemeListUnique' || type === 'MapgisUiExplosionUnique'
      "
    >
      <div class="mix-row-theme-list-unique-title">
        <mapgis-ui-row type="flex">
          <mapgis-ui-col v-if="type === 'MapgisUiThemeListUnique'" :span="3">
            <p
              v-if="type === 'MapgisUiThemeListUnique'"
              class="mix-row-list-p"
          ...
          <mapgis-ui-col
            :span="
              type === 'MapgisUiThemeListUnique' ||
              type === 'MapgisUiExplosionUnique'
                ? type === 'MapgisUiThemeListUnique'
                  ? 4
                  : 8
                : 2
          ...
          >
            ...
            <p
              v-if="type === 'MapgisUiExplosionUnique'"
              class="mix-row-list-p"
              style="padding-left: 12px;"
            >
              平移向量
          ...
          <mapgis-ui-col v-if="type != 'MapgisUiExplosionUnique'" :span="4">
            <p
              class="mix-row-list-p"
              :style="{
                paddingLeft: '8px'
          ...
          <mapgis-ui-col :span="6">
            <p class="mix-row-list-p" style="padding-left: 0px;">字段值</p>
          </mapgis-ui-col>
          <mapgis-ui-col :span="6">
            <p
              class="mix-row-list-p"
      ...
      <mapgis-ui-list :data-source="listProps.dataSource">
        <mapgis-ui-empty
          :image="defaultImage"
          :description="false"
          ...
          <div class="mix-row-list">
            ...
            >
              <input
                ...
                @click="
                  $_change(
                    listProps.checkBoxArr[index],
                    index,
                    listProps.colors[index]
                  )
                "
              />
            </mapgis-ui-col>
            <mapgis-ui-col :span="3" v-if="type != 'MapgisUiExplosionUnique'">
              <p
                class="mix-row-p"
                :class="{
                  mixRowPLarge: listProps.size === 'large',
                  mixRowPSmall: listProps.size === 'small'
                }"
              >
                {{ index + 1 }}
              </p>
            ...
            <mapgis-ui-col
              ...
              "
              v-if="
                (listProps.colors && listProps.colors.length > 0) ||
                  (listProps.directions && listProps.directions.length > 0)
              "
            >
              <mapgis-ui-sketch-color-picker
                v-if="type === 'MapgisUiThemeListUnique'"
              ...
              <mapgis-ui-input
                v-if="type === 'MapgisUiExplosionUnique'"
                v-model="listProps.directions[index]"
                :placeholder="inputProps.placeholder"
                :addonAfter="inputProps.addonAfter"
                :addonBefore="inputProps.addonBefore"
                :disabled="disabled"
            ...
            <mapgis-ui-col :span="6" v-if="type != 'MapgisUiExplosionUnique'">
              <p
                class="mix-row-p"
                :class="{
                  mixRowPLarge: listProps.size === 'large',
            ...
            <mapgis-ui-col :span="6" style="display:flex">
              <img
                class="mix-row-add"
                :class="{
                  mixRowImgSmall: listProps.size === 'small',
                  mixRowLarge: listProps.size === 'large'
                }"
    ...
    >
      <div class="mix-row-theme-list-title">
        <mapgis-ui-row type="flex">
          <mapgis-ui-col
            :span="
              type === 'MapgisUiThemeList' || type === 'MapgisUiExplosionRange'
                ? type === 'MapgisUiThemeList'
                  ? 4
                  : 6
                : 2
          ...
          >
            ...
            <p
              v-if="type === 'MapgisUiThemeListSymbol'"
              class="mix-row-list-p"
              style="padding-left: 12px;"
            >
              半径
            ...
            <p
              v-if="type === 'MapgisUiExplosionRange'"
              class="mix-row-list-p"
              style="padding-left: 12px;"
            >
              平移向量
          ...
          <mapgis-ui-col
            v-if="type != 'MapgisUiExplosionRange'"
            :span="type === 'MapgisUiThemeList' ? 4 : 4"
          >
            <p
              class="mix-row-list-p"
              :style="{
                paddingLeft: type === 'MapgisUiThemeList' ? '8px' : '8px'
              }"
            >
              字段名
            </p>
      ...
      <mapgis-ui-list :data-source="listProps.dataSource">
        ...
        <mapgis-ui-list-item slot="renderItem" slot-scope="item, index">
          <div class="mix-row-list">
            <mapgis-ui-row :class="{ mixRowSmall: listProps.size === 'small' }">
              ...
              >
                ...
                >
                  <div
                    class="mix-row-color"
                    :style="{ background: listProps.colors[index] }"
                  />
                </mapgis-ui-sketch-color-picker>
                <div
                  v-if="type === 'MapgisUiThemeListSymbol'"
                  class="mix-row-radius-img-out"
                >
                  <img
                    class="mix-row-radius-img"
              ...
              <mapgis-ui-col :span="4" v-if="type != 'MapgisUiExplosionRange'">
                <p
                  class="mix-row-p"
            ...
            <mapgis-ui-row
              v-show="listProps.radiusArray[index]"
              :class="{ mixRowSmall: listProps.size === 'small' }"
            >
              <mapgis-ui-col :span="3">
                <p
                  :style="{
                    paddingBottom: listProps.size === 'small' ? 'none' : '1em'
                  }"
                  style="padding-left: 12px"
                >
                  半径
  ...
  methods: {
    ...
    $_initProps() {
      switch (this.type) {
        ...
        case "MapgisUiColorPicker":
          this.colorPickerProps = Object.assign(
            this.colorPickerProps,
            this.props
          );
          break;
        case "MapgisUiGrediantSelect":
          this.grediantPickerProps = Object.assign(
            this.grediantPickerProps,
            this.props
          );
          if (
            !this.valueCopy &&
            this.grediantPickerProps.gradients.length > 0
          ) {
        ...
        case "MapgisUiInputNumber":
          this.inputNumberProps = Object.assign(
        ...
        case "MapgisUiThemeListUnique":
          ...
          this.listProps.colors = this.$_getUniqueColors(
            this.listProps.gradient,
        ...
        case "MapgisUiExplosionRange":
          this.listProps = Object.assign(this.listProps, this.props);
          this.listProps.rangeLevel = this.listProps.directions.length;
          this.$_setThemeListDataSource("range");
          this.$nextTick(function() {
            let panel = document.getElementById(this.panelId);
            this.panelWidth = panel.offsetWidth;
    ...
    $_change(e, index, extra) {
      ...
      if (this.type === "MapgisUiThemeList") {
        ...
        this.$emit(
          "change",
          "MapgisUiThemeListCheckBox",
          !e,
          index,
          this.listProps.checkBoxArr,
          extra
        );
      } else {
    ...
    $_inputChange(index) {
      if (index === 0) {
        if (
          this.listProps.dataSource[index] <= this.listProps.startData ||
          this.listProps.dataSource[index] >=
            this.listProps.dataSource[index + 1]
        ) {
        }
    ...
    $_sliderChange(index) {
      ...
      this.$nextTick(() => {
        this.$set(
          this.listProps.radiusArray,
          index,
          !this.listProps.radiusArray[index]
        );
      });
    },
    $_addUnique(index) {
      this.listProps.dataSource.splice(
        index + 1,
        0,
        this.listProps.dataSource[index]
      );
      this.listProps.colors.splice(index + 1, 0, this.listProps.colors[index]);
      this.listProps.directions.splice(
        index + 1,
        0,
    ...
    $_deleteUnique(index) {
      if (this.listProps.dataSource.length > 2) {
    ...
    $_addRange(index) {
      ...
      if (index < this.listProps.dataSource.length - 1) {
        if (startData < endData) {
          ...
          if (this.type != "MapgisUiExplosionRange") {
            let newColors = gradientColor(
              this.listProps.colors[index],
              this.listProps.colors[index + 1],
              2
            );
