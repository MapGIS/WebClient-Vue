    ...
    <div class="svg-select-panel" v-show="showPanel">
      <div class="svg-select-panel-head">
        ...
        <div class="svg-select-panel-head-tabs">
          <div id="translateId" class="svg-select-panel-head-translateX" :style="{transform: 'translateX(' + translateX + 'px)'}">
            <div :class="{svgActiveType: activeType === iconObj.type}" :id="iconObj.type" @click="$_changeType(iconObj.type)" class="svg-select-panel-tab" v-for="(iconObj,index) in icons" :key="index">
              {{iconObj.type}}
            </div>
          </div>
        </div>
