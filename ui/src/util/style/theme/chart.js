...
export const handleMultiGradient = (colorGroupsData, dataLength) => {
  ...
  startColorGroups = SuperMap.ColorsPickerUtil.getGradientColors(
    startColors,
    dataLength,
    "RANGE"
  );
  endColorGroups = SuperMap.ColorsPickerUtil.getGradientColors(
    endColors,
    dataLength,
    "RANGE"
  );
...
export const getMultiColorGroup = (colorGroup, dataNumber) => {
  ...
  if (
    colorGroup &&
    dataNumber > colorGroup.length &&
    typeof colorGroup[0] === "object"
  ) {
    nextColorGroup = handleMultiGradient(colorGroup, dataNumber);
  } else {
    nextColorGroup = SuperMap.ColorsPickerUtil.getGradientColors(
      colorGroup,
      dataNumber,
      "RANGE"
    );
