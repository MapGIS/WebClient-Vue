...
export default {
  ...
  data() {
    return {
      GjsonSize:
        window.localStorage.getItem("mapgis_clouddisk_GjsonSize") || 1000,
      vectorTileSize:
        window.localStorage.getItem("mapgis_clouddisk_vectorTileSize") ||
        500000,
      onlyOneField:
        window.localStorage.getItem("mapgis_clouddisk_onlyOneField") || false,
      mapstudioUrlMark:
  ...
  methods: {
    openStyle() {
      ...
      if (this.selectStyle.url) {
        ...
        if (this.selectStyle.isfolder === true) {
          ...
          let downloadFileUrl = getFileDownloadUrlWithAuth(
            this.selectStyle.url,
            false
          );
          axios
            .get(downloadFileUrl)
    ...
    addLayer() {
      ...
      for (let i = 0; i < this.selectLists.length; i++) {
        ...
        if (!xattrs) {
          this.$notification.error({
            message: "当前选择的某个图层缺少图层信息",
            description: "请检查所选图层是否导入！"
          });
          continue;
        }
        let {
        ...
        useVectorTile = renderType
          ? renderType === "vectorTile"
          : vclsinfo && vclsinfo.count > this.GjsonSize;
        // console.warn('检验useVectorTile', useVectorTile)
        if (vclsinfo && useVectorTile) {
          // 实时矢量瓦片
          if (hasVector && dataSource) {
            let temp = [];
            temp = getCacheVectorTileUrl(
              storeServiceUrl,
              dataSourceType,
              dataSource,
              4326,
          ...
          } else if (!hasVector && tileDataPath) {
            // 镶嵌数据集
            // path = getTileUrl(storeServiceUrl, tileDataPath)
            if (layer.type === FileType.TIF || layer.type === FileType.TIFF) {
              path =
                getTiffUrl(storeServiceUrl, tileDataPath, 4326) +
                `&Authorization=${this.cdToken}`;
              metaPath = getTiffMetaUrl(storeServiceUrl, tileDataPath, 0, 0);
            } else {
              path =
                getNewTileUrl(storeServiceUrl, tileDataPath, dataSource, 3857) +
                `&Authorization=${this.cdToken}`;
            }
          } else {
            this.$notification.error({
      ...
      layers.forEach(item => {
        let promiseFunction;
        if (
          item.type !== FileType.MOSA &&
          item.type !== FileType.TIF &&
          item.type !== FileType.TIFF
        ) {
            ...
            if (res.status === 200) {
              ...
              if (errorCode < 0) {
                ...
                if (item.isVectorTile) {
                  keepField =
                    this.onlyOneField === true ? fields[0].name : keepField; // 只取所有属性字段的第一个，这两个二选一
                  item.path =
                    item.path1 +
                    keepField +
                    item.path2 +
    ...
    getTiffMetadata(item) {
      // 该方法只是将原layer返回，后面若需要加tiff或镶嵌数据集的图层范围等信息可写在此处
      return new Promise((resolve, reject) => {
        resolve(item);
      });
    },
    ...
    checkDuplication(uuid) {
      let flag = false;
      if (
        this.currentDocument.layers &&
        this.currentDocument.layers.length > 0
      ) {
