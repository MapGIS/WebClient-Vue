...
Storybook帮助您构建与应用程序的业务逻辑、数据和上下文隔离的UI组件。
这使得容易控制组件的状态以及监控变化。将这些UI状态保存为“故事”，以便在开发、测试期间重新复现。
现在可以通过在侧边栏中导航来浏览示例故事。
查看“src/stories”目录下的代码，了解它们是如何工作的。
我们建议使用一个[**组件驱动的**](https://componentdriven.org)过程来构建ui，该过程从原子组件开始，以页面结束。
