export default {
  title: "二维/可视化/专题图/标签/多边形/IGS生成标签(10.5.5以后可用)",
  argTypes: {
    dataSource: {
      description:
        "使用IGS服务时，请传入一个IGS获取要素的服务地址，例如：'http://localhost:6163/igs/rest/mrfs/layer/query?f=json&gdbp=gdbp://MapGISLocalPlus/wuhan_new/sfcls/武汉市&structs={IncludeAttribute:true,IncludeGeometry:true,IncludeWebGraphic:false}&pageCount=200&page=0&cursorType=forward&rtnLabel=true'<br>" +
        "使用此服务是需要注意以下几点：<br>" +
    ...
    type: {
      description:
        "<span class='storybook-span'>type</span>(必填)：专题图类型，类型有以下值，uniform(统一)，unique(单值)，range(分段)，heatmap(热力)，symbol(符号)",
      table: {
        defaultValue: { summary: "null" },
      },
    },
    field: {
      description:
        "<span class='storybook-span'>field</span>(必填)：属性字段，即以某个字段的值来创建专题图",
      table: {
        defaultValue: { summary: "null" },
      },
    },
    themeOptions: {
      description:
        "专题图样式，包含专题图图层样式、分段样式以及标签样式，其中标签样式如下：<br>" +
  ...
  data() {
    return {
      mapStyle: {
        ...
        glyphs: `${window.domain}/${window.glyphs}/{fontstack}/{range}.pbf`,
      },
    };
  },
});
