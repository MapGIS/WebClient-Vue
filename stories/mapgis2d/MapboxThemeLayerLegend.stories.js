...
export default {
  title: "二维/可视化/专题图/开启图例",
  argTypes: {
    dataSource: {
      description:
        "geojson格式的数据源或者geojson数据源的URl，详见如下网址：<a href='https://geojson.org/' target='_blank'>https://geojson.org/  </a>",
      table: {
    ...
    themeOptions: {
      description:
        "专题图样式，包含专题图图层样式、分段样式以及标签样式，样式如下：<br>" +
      ...
      table: {
        type: {
          ...
          detail: formatObjectToHtml({
            title: "图例标题,String对象",
            fields:
              "图例里面要显示的字段，Array对象，请与专题图的渐变颜色数量保持一致",
            style: {
...
const Template = (args, { argTypes }) => ({
  ...
  methods: {},
  template: `
      <mapgis-web-map crs="EPSG:4326" :mapStyle="mapStyle" :center="[114.299039,30.594797]" :zoom="8"
                      style="height:96vh">
