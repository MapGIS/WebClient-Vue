import { DomUtil, Util, Layer, LatLng, stamp } from "leaflet";
 ...
 * @param options - {Object} Util.setOptions(this, options);只做额外增加的字段作用<br>
 ...
 * var options = {
    ...
    var mapvLayer = new MapvLayer(map, dataSet, options).addTo(map);
 */
export var MapvLayer = Layer.extend({
  ...
  onAdd: function (map) {
    ...
    var _container = DomUtil.create("div", "leaflet-layer", tilePane);
  ...
  _animateZoom: function (e) {
    ...
    //bg.style[transform] =  DomUtil.getTranslateString(origin) + ' scale(' + e.scale + ') ';
  },
  ...
  fixPosition: function () {
    /* var domPosition = DomUtil.getPosition(this._map.getPanes().mapPane);
        ...
        if (domPosition) {
            DomUtil.setPosition(this._canvas, {
                x: -domPosition.x,
                y: -domPosition.y
            });
        } */
    ...
    // var zero = this.map.latLngToLayerPoint(LatLng(0, 0));
